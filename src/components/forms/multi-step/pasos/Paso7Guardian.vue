  ...
  <div class="step-guardian">
    ...
    >
      <div class="form-grid">
        ...
        <div class="form-group" v-if="isConsentTrue">
          <FormKit
            type="date"
            name="consent_date"
          ...
          <div class="consent-summary">
            <div class="summary-item">
              <div class="summary-icon" :class="consentIconClass">
                <CheckCircleIcon v-if="isConsentTrue" class="h-5 w-5" />
                <XCircleIcon v-else-if="isConsentFalse" class="h-5 w-5" />
...
interface Props {
  formData: {
    guardian_consent: string | boolean
    consent_date: string
  }
}
...
// ✅ HELPERS PARA NORMALIZAR VALORES (Resuelve inconsistencia de tipos string/boolean)
const isConsentTrue = computed(() => {
  const consent = localData.value.guardian_consent
  return String(consent) === 'true'
})

const isConsentFalse = computed(() => {
  const consent = localData.value.guardian_consent
  return String(consent) === 'false'
})
...
const consentStatusText = computed(() => {
  if (isConsentTrue.value) return 'Consentimiento OTORGADO'
  if (isConsentFalse.value) return 'Consentimiento DENEGADO'
  return 'Pendiente de respuesta'
})

const consentIconClass = computed(() => {
  if (isConsentTrue.value) return 'status-approved'
  if (isConsentFalse.value) return 'status-denied'
...
const formatConsentDate = computed(() => {
  ...
  const date = new Date(localData.value.consent_date + 'T12:00:00') // Evita problemas de zona horaria
...
const consentImplications = computed(() => {
  if (isConsentTrue.value) {
    return 'El niño/niña puede recibir evaluación e intervención completa'
  }
  if (isConsentFalse.value) {
    return 'No se puede proceder con la evaluación e intervención'
  }
  return 'Es necesario el consentimiento para continuar'
})

const showConsentDocument = computed(() => isConsentTrue.value)

const showConsentSummary = computed(() => {
  return localData.value.guardian_consent !== '' && localData.value.guardian_consent !== null
})

// Estado general
const overallStatusText = computed(() => {
  if (isConsentTrue.value) return 'Autorizado para evaluación e intervención'
  if (isConsentFalse.value) return 'No autorizado - Requiere revisión'
...
const isStepValid = computed(() => {
  const hasConsentValue = localData.value.guardian_consent !== '' && localData.value.guardian_consent !== null
  
  if (isConsentTrue.value) {
    // Si otorga consentimiento, la fecha es estrictamente obligatoria
    return hasConsentValue && localData.value.consent_date !== ''
  }
  
  // Si marca "No" o no ha respondido, evaluamos si ya seleccionó una opción
  return hasConsentValue
})

// Maneja cambios en el formulario
const handleFormChange = () => {
  // Limpieza lógica de fecha si cambia a "No"
  if (isConsentFalse.value) {
    localData.value.consent_date = ''
  }

  // ✅ UX IMPROVEMENT: Poner fecha actual automáticamente si marca "Sí" y está vacía
  if (isConsentTrue.value && !localData.value.consent_date) {
    localData.value.consent_date = new Date().toISOString().split('T')[0]
  }
...
onMounted(() => {
  if (props.formData) {
    localData.value = {
      guardian_consent: String(props.formData.guardian_consent),
...
// Watch para cambios en los props (sincronización con el padre)
watch(() => props.formData, (newData) => {
  if (newData) {
    localData.value = {
      guardian_consent: String(newData.guardian_consent),
...
<style scoped>
/* Estilos originales mantenidos y optimizados */
.step-guardian { @apply space-y-6; }
...
.consent-info { @apply flex items-start space-x-3 p-4 bg-green-50 border border-green-200 rounded-lg; }
.info-content { @apply flex-1; }
.info-title { @apply text-sm font-medium text-green-900 mb-2; }
.info-list { @apply text-sm text-green-700 space-y-1; }
.consent-document { @apply border border-gray-200 rounded-lg overflow-hidden; }
.document-header { @apply bg-blue-50 px-4 py-3 border-b border-blue-200 flex items-center space-x-3; }
.document-title { @apply text-lg font-semibold text-blue-900; }
.document-content { @apply p-4 bg-white; }
.document-text { @apply text-gray-700 mb-4; }
.document-list { @apply space-y-2 mb-4 text-gray-700; }
.document-list li { @apply ml-4; }
.document-footer { @apply pt-4 border-t border-gray-200 space-y-2; }
.document-date { @apply text-sm text-gray-600; }
.document-status { @apply text-sm font-medium; }
.document-status.status-approved { @apply text-green-600; }
.document-status.status-denied { @apply text-red-600; }
.document-status.status-pending { @apply text-yellow-600; }
.consent-summary { @apply space-y-4 p-4 bg-gray-50 border border-gray-200 rounded-lg; }
.summary-item { @apply flex items-start space-x-3; }
.summary-icon { @apply flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center; }
.summary-icon.status-approved { @apply bg-green-100 text-green-600; }
.summary-icon.status-denied { @apply bg-red-100 text-red-600; }
.summary-icon.status-pending { @apply bg-yellow-100 text-yellow-600; }
...
.summary-description { @apply block text-xs text-gray-600 mt-1; }
.summary-status { @apply flex items-center space-x-2 p-3 rounded-lg border text-sm font-medium; }
.summary-status.status-approved { @apply bg-green-50 border-green-200 text-green-800; }
.summary-status.status-denied { @apply bg-red-50 border-red-200 text-red-800; }
.summary-status.status-pending { @apply bg-yellow-50 border-yellow-200 text-yellow-800; }
.status-text { @apply font-medium; }
...
<style>
/* Estilos FormKit consistentes */
.guardian-form-grid { @apply space-y-6; }
.formkit-outer { @apply mb-0; }
.formkit-label { @apply block text-sm font-medium text-gray-700 mb-2; }
.formkit-legend { @apply block text-sm font-medium text-gray-700 mb-3; }
.formkit-fieldset { @apply space-y-3; }
.formkit-radio-wrapper { @apply flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors duration-150 cursor-pointer; }
.formkit-radio-input { @apply h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300; }
.formkit-radio-label { @apply block text-sm font-medium text-gray-700 cursor-pointer; }
...
.formkit-radio-wrapper[data-checked="true"] { @apply bg-blue-50 border-blue-200; }
</style>